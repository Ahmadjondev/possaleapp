...
bool Webview::MoveFocus(int reason) {
  if (!IsValid()) {
    return false;
  }

  COREWEBVIEW2_MOVE_FOCUS_REASON focus_reason;
  switch (reason) {
    case 1:
      focus_reason = COREWEBVIEW2_MOVE_FOCUS_REASON_NEXT;
      break;
    case 2:
      focus_reason = COREWEBVIEW2_MOVE_FOCUS_REASON_PREVIOUS;
      break;
    default:
      focus_reason = COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC;
      break;
  }

  return SUCCEEDED(webview_controller_->MoveFocus(focus_reason));
}
