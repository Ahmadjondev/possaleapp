...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Moves focus to the WebView2 control.
  ///
  /// [reason]: 0 = Programmatic, 1 = Next, 2 = Previous
  /// See: https://learn.microsoft.com/en-us/microsoft-edge/webview2/reference/win32/icorewebview2controller#movefocus
  Future<void> moveFocus({int reason = 0}) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod('moveFocus', reason);
  }
