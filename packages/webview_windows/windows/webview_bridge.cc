...
void WebviewBridge::HandleMethodCall(
  ...
  // moveFocus: int (0=Programmatic, 1=Next, 2=Previous)
  if (method_name.compare(kMethodMoveFocus) == 0) {
    if (const auto reason = std::get_if<int32_t>(method_call.arguments())) {
      if (webview_->MoveFocus(*reason)) {
        return result->Success();
      }
      return result->Error(kMethodFailed, "Moving focus failed.");
    }
    return result->Error(kErrorInvalidArgs);
  }
