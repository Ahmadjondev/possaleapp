...
class EscPosBuilder {
  ...
  /// Print a raster bit image using GS v 0 command.
  ///
  /// [widthBytes] – image width in bytes (8 pixels per byte).
  /// [height] – image height in dots/pixels.
  /// [data] – raw 1-bit/pixel bitmap, MSB = leftmost, 1 = black.
  EscPosBuilder rasterImage(int widthBytes, int height, Uint8List data) {
    // GS v 0 m xL xH yL yH d1...dk
    _buffer.addAll([0x1D, 0x76, 0x30, 0x00]); // m=0 (normal density)
    _buffer.addAll([widthBytes & 0xFF, (widthBytes >> 8) & 0xFF]); // xL xH
    _buffer.addAll([height & 0xFF, (height >> 8) & 0xFF]); // yL yH
    _buffer.addAll(data);
    return this;
  }

  /// Paper width in dots (203 DPI). 80mm → 576, 57mm → 384.
  int get paperWidthDots => charsPerLine >= 48 ? 576 : 384;
